...
def display_world(world_size, position, landmarks=None, r=0):
    ...
    if r: 
        ax.text(r.x, r.y, 'o', ha='center', va='center', color='g', fontsize=30)
        #if(landmarks is not None):
        # loop through all path indices and draw a dot (unless it's at the car's location)
        for pos in r.landmarks:
            if(pos != position):
                ax.text(pos[0], pos[1], 'x', ha='center', va='center', color='g', fontsize=20)
